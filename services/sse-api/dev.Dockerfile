...
# System deps:
RUN apt-get update \
    && apt-get install -y gcc g++ unzip wget procps htop ffmpeg libavcodec-extra libsndfile1 libwebp \
...
# FOR LOCAL DEVELOPMENT ENVIRONMENT
# Install the sse-api package.
# Mapping a volume to ./services/sse-api/src is required when running this image.
ENTRYPOINT ["/bin/sh", "-c" , "poetry install --only-root && poetry run python src/sse_api/main.py"]
