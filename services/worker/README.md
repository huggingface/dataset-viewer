...
- `DUCKDB_INDEX_MAX_SPLIT_SIZE_BYTES`: size in bytes of raw uncompressed data by which the number of first `n` parquet files to index is determined, satisfies the following condition: `sum(parquet_files[0] + ... + parquet_files[n-1]) <= DUCKDB_INDEX_MAX_SPLIT_SIZE_BYTES & sum(parquet_files[0] + ... + parquet_files[n]) > DUCKDB_INDEX_MAX_SPLIT_SIZE_BYTES`. Defaults to `100_000_000`.
...
- `DESCRIPTIVE_STATISTICS_MAX_SPLIT_SIZE_BYTES`: size in bytes of raw uncompressed data by which the number of first `n` parquet files to compute statistics over is determined, satisfies the following condition: `sum(parquet_files[0] + ... + parquet_files[n-1]) <= DESCRIPTIVE_STATISTICS_MAX_SPLIT_SIZE_BYTES & sum(parquet_files[0] + ... + parquet_files[n]) > DESCRIPTIVE_STATISTICS_MAX_SPLIT_SIZE_BYTES`. Defaults to `100_000_000`.
