...
- `FIRST_ROWS_MAX_BYTES`: the max size of the /first-rows job response in bytes. Defaults to `1_000_000` (1 MB).
- `FIRST_ROWS_MAX_NUMBER`: the max number of rows fetched by the worker for the split and provided in the /first-rows job response. Defaults to `100`.
