# build with
#   docker build -t some_tag_filter -f Dockerfile ../..
...
COPY services/filter/poetry.lock ./services/filter/poetry.lock
COPY services/filter/pyproject.toml ./services/filter/pyproject.toml
...
# Mapping a volume to ./services/filter/src is required when running this image.
ENTRYPOINT ["/bin/sh", "-c" , "poetry install --only-root && poetry run python src/filter/main.py"]
