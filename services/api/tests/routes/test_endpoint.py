...
def test_endpoints_definition() -> None:
    ...
    opt_in_out_ursl = definition["/opt-in-out-urls"]
    assert opt_in_out_ursl is not None
    assert sorted(list(opt_in_out_ursl)) == ["split"]
    assert opt_in_out_ursl["split"] is not None
    assert len(opt_in_out_ursl["split"]) == 1  # Only has one processing step
