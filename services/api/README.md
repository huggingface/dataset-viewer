...
- `API_HF_AUTH_BYPASS_PUBLIC_KEY`: the public key used to decode the JWT that can be passed in the 'X-Api-Key' header to bypass the external authentication check. If not set, we don't check if a JWT has been passed in the request. Defaults to empty.
