...
from api.utils import (
    ...
    ParquetResponseNotFoundError,
    ParquetResponseNotReadyError,
    UnexpectedError,
...
def create_parquet_endpoint(
    ...
    async def parquet_endpoint(request: Request) -> Response:
        try:
            dataset = request.query_params.get("dataset")
            logging.info(f"/parquet, dataset={dataset}")
            ...
            try:
                result = get_response(kind=CacheKind.PARQUET.value, dataset=dataset)
