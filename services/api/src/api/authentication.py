...
def auth_check(
    ...
    try:
        ...
        raise ExternalAuthCheckUrlFormatError("external_auth_url must contain %s") from e
    try:
        response = requests.get(url, auth=RequestAuth(request))
    except Exception as err:
        raise ExternalAuthCheckConnectionError("External authentication check failed", err) from err
    if response.status_code == 200:
        ...
        raise ExternalAuthCheckResponseError(f"Unexpected status code {response.status_code}")
