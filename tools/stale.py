...
LABELS_TO_EXEMPT_IN_LOWERCASE = [label.lower() for label in ["P0", "P1", "P2"]]
...
def main():
    ...
    for issue in open_issues:
        ...
        if (now - issue.created_at).days < 30 or any(
            label.name.lower() in LABELS_TO_EXEMPT_IN_LOWERCASE
        ...
        elif (now - issue.updated_at).days > 23:
            # add stale comment
            issue.create_comment(
                "This issue has been automatically marked as stale because it has not had "
                "recent activity. If you think this still needs to be addressed "
                "please comment on this thread.\n\nPlease note that issues that do not follow the "
                "[contributing guidelines](https://github.com/huggingface/dataset-viewer/blob/main/CONTRIBUTING.md) "
