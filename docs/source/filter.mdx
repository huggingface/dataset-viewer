...
The `/filter` endpoint accepts five query parameters:
...
```
where=age=30 AND name='Simone'
```
will filter the data to select only those rows where the 'age' column is equal to 30 and the 'name' column is equal to 'Simone'.
