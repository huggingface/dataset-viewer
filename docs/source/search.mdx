...
<Tip warning={true}>
  ...
  are supported so Datasets Server can index the contents and run the search without downloading the
  whole dataset.
</Tip>
...
The text is searched in the columns of type `string` or `ClassLabel`, even if the values are nested in a dictionary.
...
The rows are ordered by the row index. Note that there is no indication of the pertinence of a specific row, and the matching strings are not highlighted.
