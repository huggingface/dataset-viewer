...
For example, here are the columns and the first rows of the `duorc` / `SelfRC` train split.

https://datasets-server.huggingface.co/rows?dataset=duorc&config=SelfRC&split=train
...
https://datasets-server.huggingface.co/rows?dataset=ett&config=m2&split=test
