...
<Tip warning={true}>
  ...
  dataset may take hours! See the `preview` field in the response of `/valid`.
</Tip>
...
- `valid` (_deprecated_): the union of the `viewer` and `preview` lists.
...
```json
{
  ...
  "valid": [
    "// [deprecated] contains the union of 'viewer' and 'preview'."
  ]
}
```
