...
Parquet is a columnar storage format that is optimized for querying and processing large datasets. Parquet is a popular choice for big data processing and analytics, and it is widely used in the industry.
...
- only the necessary columns are read from disk (projection pushdown), no need to read the entire file
...
The Parquet files are published to the Hub on a specific `refs/convert/parquet` branch (like this `amazon_polarity` [branch](https://huggingface.co/datasets/amazon_polarity/tree/refs%2Fconvert%2Fparquet) for example) that lives in parallel to the `main` branch.
