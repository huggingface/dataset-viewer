...
In order for Datasets Server to generate a Parquet version of a dataset, the dataset must be _public_, or owned by a [PRO user](https://huggingface.co/pricing) or an [Enterprise organization](https://huggingface.co/enterprise).
