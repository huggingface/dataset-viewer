...
It takes the dataset name, the configuration name and the split as parameters:
...
The endpoint response is a JSON containing a list of the dataset's files URLs in the Parquet format. For example, the [`duorc`](https://huggingface.co/datasets/duorc) dataset has one Parquet file for the train split of  "ParaphraseRC" configuration (see the [List splits and configurations](./splits) guide for more details about splits and configurations). 

```json
["https://huggingface.co/datasets/duorc/resolve/refs%2Fconvert%2Fparquet/ParaphraseRC/duorc-train.parquet"]
```
