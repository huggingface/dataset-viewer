...
This returns a list of all the datasets that load without an error for the viewer (/rows) or only the preview (/first-rows):
...
```json
{
  ...
  "preview": [
    "0xMonolith/PixelArtConcepts_1",
	"13GP/training",
	"2413016570gjh/vae_zip",
...
The `/search` endpoint returns a JSON list of a slice of rows of a dataset that match a text query. The text is searched in the columns of type `string` or `ClassLabel`, even if the values are nested in a dictionary.
