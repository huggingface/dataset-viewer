# Explore statistics over split's data
...
* `statistics` - list of dictionaries for statistics per each column, each dictionary has three keys: `column_name`, `column_type`, and `column_statistics`. Content of `column_statistics` depends on a column type. See section ... for more details
...
The following statistics are returned for float data types:
...
The following statistics are returned for integer data types:
...
If string column has less than or equal to 30 unique values withing the requested split, it is considered to be a category. The following measures are computed:
...
If string column has more than 30 unique values withing the requested split, it is considered to be a column containing texts and respons contains statistics over text lengths. The following measures are computed:
