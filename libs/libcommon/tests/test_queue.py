...
from libcommon.dtos import Priority, Status, WorkerType
from libcommon.queue import (
    ...
    Lock,
    Queue,
    WorkerTypeJobCountDocument,
    lock,
)
from libcommon.resources import QueueMongoResource
from libcommon.utils import get_datetime

from .utils import assert_metric, assert_worker_type_jobs_count
...
def test_add_job() -> None:
    ...
    assert WorkerTypeJobCountDocument.objects().count() == 0
    # add a job
    ...
    assert_worker_type_jobs_count(worker_type=WorkerType.MEDIUM, jobs_count=1)
    ...
    assert_worker_type_jobs_count(worker_type=WorkerType.MEDIUM, jobs_count=2)
    ...
    assert_worker_type_jobs_count(worker_type=WorkerType.MEDIUM, jobs_count=0)
...
def test_delete_waiting_jobs_by_job_id(
    ...
    for job_id in all_jobs:
        ...
        assert_worker_type_jobs_count(worker_type=WorkerType.MEDIUM, jobs_count=waiting_jobs)
    ...
    assert_worker_type_jobs_count(worker_type=WorkerType.MEDIUM, jobs_count=len(all_jobs))

    queue.start_job()
    assert_metric(job_type=test_type, status=Status.WAITING, total=len(all_jobs) - 1)
    assert_metric(job_type=test_type, status=Status.STARTED, total=1)
    assert_worker_type_jobs_count(worker_type=WorkerType.MEDIUM, jobs_count=len(all_jobs) - 1)
