...
def test_add_job() -> None:
    ...
    assert_metric(queue=test_type, status=Status.WAITING, total=1)
    ...
    assert_metric(queue=test_type, status=Status.WAITING, total=2)
    ...
    assert_metric(queue=test_type, status=Status.CANCELLED, total=1)
    assert_metric(queue=test_type, status=Status.STARTED, total=1)
    ...
    assert queue.is_job_in_process(job_type=test_type, dataset=test_dataset, revision=test_revision)
    assert_metric(queue=test_type, status=Status.WAITING, total=1)
    assert_metric(queue=test_type, status=Status.STARTED, total=0)
    assert_metric(queue=test_type, status=Status.CANCELLED, total=1)
    assert_metric(queue=test_type, status=Status.SUCCESS, total=1)

    # process the third job
    job_info = queue.start_job()
    other_job_id = ("1" if job_info["job_id"][0] == "0" else "0") + job_info["job_id"][1:]
    assert_metric(queue=test_type, status=Status.WAITING, total=0)
    assert_metric(queue=test_type, status=Status.STARTED, total=1)
    assert_metric(queue=test_type, status=Status.CANCELLED, total=1)
    assert_metric(queue=test_type, status=Status.SUCCESS, total=1)
    ...
    assert_metric(queue=test_type, status=Status.STARTED, total=0)
    assert_metric(queue=test_type, status=Status.CANCELLED, total=1)
    assert_metric(queue=test_type, status=Status.SUCCESS, total=2)
...
def test_cancel_jobs_by_job_id(
    ...
    for job_id in list(set(jobs_ids + job_ids_to_cancel)):
        ...
        waiting_jobs += 1
        assert_metric(queue=test_type, status=Status.WAITING, total=waiting_jobs)
    ...
    assert_metric(queue=test_type, status=Status.CANCELLED, total=expected_canceled_number)
