...
def test_default_graph() -> None:
    ...
    assert_step(config_names, parent=None, children=[split_names_from_streaming], ancestors=[])
    assert_step(
    ...
    assert_step(
        parquet_and_dataset_info, parent=None, children=[config_parquet, config_info, config_size], ancestors=[]
    )
    assert_step(
        config_parquet,
        parent=parquet_and_dataset_info,
        children=[split_first_rows_from_parquet, dataset_parquet],
        ancestors=[parquet_and_dataset_info],
    )
    assert_step(
        dataset_parquet, parent=config_parquet, children=[], ancestors=[parquet_and_dataset_info, config_parquet]
    )
    assert_step(
        config_info,
        parent=parquet_and_dataset_info,
    ...
    assert_step(dataset_info, parent=config_info, children=[], ancestors=[parquet_and_dataset_info, config_info])
    assert_step(
        config_size, parent=parquet_and_dataset_info, children=[dataset_size], ancestors=[parquet_and_dataset_info]
    )
    assert_step(dataset_size, parent=config_size, children=[], ancestors=[parquet_and_dataset_info, config_size])
    assert_step(
