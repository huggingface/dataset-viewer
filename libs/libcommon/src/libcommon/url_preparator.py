    ...
    def _prepare_asset_url_path_in_place(self, cell: Any, asset_url_path: AssetUrlPath, revision: str) -> Any:
        if not cell:
            ...
            if not isinstance(cell, dict):
                raise InvalidFirstRowsError(
                    f"Expected the cell at path {asset_url_path.path} to be a dict but it's a {type(cell)}"
                )
            for key, value in cell.items():
                if isinstance(value, dict):
                    ...
                    if not isinstance(src, str):
                        raise InvalidFirstRowsError(
                            f'Expected cell["{key}"] at path {asset_url_path.path} to be a string but it\'s a {type(cell)}'
                        )
            ...
            if key == 0:
                ...
                if not isinstance(cell, list):
                    raise InvalidFirstRowsError(
                        f"Expected the cell at path {asset_url_path.path} to be a list but it's a {type(cell)}"
                    )
