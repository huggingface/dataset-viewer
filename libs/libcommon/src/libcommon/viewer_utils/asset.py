...
from typing import Any, Generator, List, Optional, Tuple, TypedDict
...
def create_asset_dir(
    ...
    if row_idx is not None:
        dir_path = dir_path / str(row_idx)
        url_dir_path = f"{url_dir_path}/{row_idx}"
        if column is not None:
            dir_path = dir_path / column
            url_dir_path = f"{url_dir_path}/{column}"
    makedirs(dir_path, ASSET_DIR_MODE, exist_ok=True)
    return dir_path, url_dir_path


def create_cell_asset_dir(
    dataset: str, config: str, split: str, row_idx: int, column: str, assets_directory: StrPath
) -> Tuple[Path, str]:
    return create_asset_dir(
        assets_directory=assets_directory, dataset=dataset, config=config, split=split, row_idx=row_idx, column=column
    )


def create_split_asset_dir(dataset: str, config: str, split: str, assets_directory: StrPath) -> Tuple[Path, str]:
    return create_asset_dir(assets_directory=assets_directory, dataset=dataset, config=config, split=split)
...
def create_image_file(
    ...
    dir_path, url_dir_path = create_cell_asset_dir(
        dataset=dataset,
        config=config,
        split=split,
        row_idx=row_idx,
...
def create_csv_file(
    ...
    """Create a csv file for a split.
    Note that it is assumed that headers is consistent with data content (same set of values and same order)
    It will create a folder based on input parameters and will create the csv inside it.
    Args:
        dataset (`str`): The dataset for the folder creation.
        config (`str`): The config for the folder creation.
        split (`str`): The config for the folder creation.
        headers (`List[str]`): The headers for the csv file.
        data (`str`): The data content for the csv file.
        assets_base_url (`str`): The base url for the assets creation.
        file_name (`str`): The file name for the csv file.
        assets_directory(`str`): The assets directory in which to create the folder for the csv.

    Returns: FileSource with src location and file type
    """
    dir_path, url_dir_path = create_split_asset_dir(
        dataset=dataset,
        config=config,
        split=split,
        assets_directory=assets_directory,
