...
def get_assets_dir(
    ...
    dir_path = (
        Path(assets_directory).resolve()
        / dataset
        / (DATASET_SEPARATOR + dataset_separator_suffix)
        / config
        / split
        / str(row_idx)
        / column
    )
    url_dir_path = f"{dataset}/{DATASET_SEPARATOR + dataset_separator_suffix}/{config}/{split}/{row_idx}/{column}"
...
def glob_rows_in_assets_dir(
    ...
    return (
        ...
        .resolve()
        .glob(os.path.join(dataset, DATASET_SEPARATOR + dataset_separator_suffix, "*", "*", "*"))
    )
...
def update_last_modified_date_of_rows_in_assets_dir(
    ...
    row_dirs_path = (
        Path(assets_directory).resolve() / dataset / (DATASET_SEPARATOR + dataset_separator_suffix) / config / split
    )
...
def create_image_file(
    ...
    dir_path, url_dir_path = get_assets_dir(
        dataset=dataset,
        config=config,
        split=split,
        row_idx=row_idx,
