...
class Worker(ABC):
    ...
    def has_memory(self) -> bool:
        if self.config.max_memory_pct <= 0:
            return True
        ...
        ok = percent < self.config.max_memory_pct
        if not ok:
            logger.info(
                f"memory usage (RAM + SWAP) is too high: {percent:.0f}% - max is {self.config.max_memory_pct}%"
            )
        return ok

    def has_cpu(self) -> bool:
        if self.config.max_load_pct <= 0:
            return True
        ...
        ok = load_pct < self.config.max_load_pct
        if not ok:
            logger.info(f"cpu load is too high: {load_pct:.0f}% - max is {self.config.max_load_pct}%")
        return ok

    def sleep(self) -> None:
        jitter = 0.75 + random.random() / 2  # nosec
        # ^ between 0.75 and 1.25
        duration = self.config.sleep_seconds * jitter
